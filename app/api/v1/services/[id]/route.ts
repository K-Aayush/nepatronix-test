...
export const GET = async (req: any) => {
  try {
    ...
    const data = await service.findOne({ link: idOfData });

    if (!data) {
      const data2 = await service.findOne({ _id: idOfData });
      console.log(data2);
...
export const DELETE = async (req: any) => {
  try {
    ...
    const data: any = await service.findOne({ _id: idOfData });
    if (!data) {
    ...
    // delete from db
    await service.deleteOne({ _id: idOfData });
...
export const PUT = async (req: any) => {
  try {
    ...
    if (newContentExist) {
      const content = await handleQuillEdit(form, "services", data?.content);
