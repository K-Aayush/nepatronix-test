...
export const GET = async (req: any) => {
  try {
    ...
    const data = await news.findOne({ link: idOfData });

    if (!data) {
      const data2 = await news.findOne({ _id: idOfData });
      console.log(data2);
...
export const DELETE = async (req: any) => {
  try {
    ...
    const data: any = await news.findOne({ _id: idOfData });
    if (!data) {
    ...
    // delete from db
    await news.deleteOne({ _id: idOfData });
...
export const PUT = async (req: any) => {
  try {
    ...
    if (newContentExist) {
      const content = await handleQuillEdit(form, "news", data?.content);
