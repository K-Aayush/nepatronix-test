...
export async function generateMetadata({ params }: { params: any }) {
  try {
    const res = await getSole("stories", params?.id);
    if (!res) {
      return {
        title: "not found",
    ...
    return {
      title: res?.title || "Client's Blog",
      description: res?.description || "Client's Blog",
      openGraph: {
        title: res?.title || "Client's Blog",
...
const page = async ({ params }: { params: any }) => {
  const data: any = await getSole("stories", params?.id);
  if (data) {
    const res = await fetch(`${domain}/api/v1/stories/more`, {
      cache: "no-store",
      method: "PUT",
      headers: {
        currentId: data?._id,
  ...
  return (
    <main className="min-h-[100vh] pt-[80px]" style={{ paddingTop: "80px" }}>
      ...
      <Ads page={"stories[id]"} index={1}/>


      {/* description  */}
      <DescriptionBlog des={data?.description} />

      <Ads page={"stories[id]"} index={2}/>
      ...
      ></div>
      <br />
      <Like likes={data?.likes} type={"stories"} id={data?._id} />
      <center>
        <BlogAddiHolder currId={data?._id} />
      </center>
      <Comment type={"stories"} id={data?._id} />
    </main>
  );
};
