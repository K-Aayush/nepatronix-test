...
const EditTeam = ({ data }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const edited = await putAdmin(formData, "teams", user?._id);
    ...
    >
      <div className="w-1/2 p-[20px] py-[100px] min-w-[350px]">
        ...
        <div className="w-full flex" style={{ gap: "20px" }}>
          ...
          <button
            type="button"
            className="w-fit bg-red-400 text-white rounded-xl"
            style={{ padding: "10px", fontSize: "18px" }}
            onClick={async () => {
              const deleteSure = window.confirm("Delete Member!");
              if (!deleteSure) return;
              const deleted = await DelData("teams", data?._id);
              if (deleted) {
      ...
      <div className="flex flex-col justify-center">
        <Image
          src={blob || `/api/files${data?.picture}`}
          width={1000}
          height={1000}
