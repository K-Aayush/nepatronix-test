...
import EditServCover from "./EditorComp/EditServCover";

const EditServ = ({ data }: { data: any }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    //post reqs
    const formReq: boolean = await putAdmin(newFormData, "services", data?._id);
    if (formReq) {
  ...
  return (
    <form className="w-full" onSubmit={handleSubmit}>
      <EditServCover oldImage={oldImage} setImage={setImage} title={title} setTitle={setTitle} />
