...
import { getMultiTasks } from "../requests/RequestData";

const TassDataProfile = () => {
  ...
  useEffect(() => {
    const get = async () => {
      const dataList = await getMultiTasks( year, month);
      ...
      <div className="w-full p-[0px] gap-[30px] flex flex-wrap">
        ...
        {datas?.map((item, index) => {
          if (item?.status === filter) {
            console.log(Object?.keys(item?.individualProgress))
            return (
              <div
              key={index}
              ...
              >
                ...
                <ul className="w-full list-disc h-[100px] overflow-hidden">
                  {Object?.keys(item?.individualProgress)?.map((prog, idx) => (
                ...
                <br />
                <Link href={`/profile/tasks/${item?._id}`}>
...
export default TassDataProfile;
