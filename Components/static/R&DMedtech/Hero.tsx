...
const Hero: React.FC<HeroProps> = ({ title, subtitle, ctaText }) => {
  return (
    <section className="relative py-32 text-white bg-gradient-to-b from-blue-900 to-gray-800 bg-opacity-10">
      <div className="flex flex-col items-center gap-8 px-6 mx-auto max-w-[1280px] md:flex-row">
        <motion.div className="md:w-1/2" variants={slideVariants}>
          <h1 className="mb-4 text-5xl font-bold md:text-7xl">{title}</h1>
          <p className="max-w-lg mb-6 text-xl md:text-2xl">{subtitle}</p>
          <Link href={"/blogs"}>
        ...
        >
          <Image
            ...
            height={1200}
            loading="eager"
            className="object-cover w-[400px] h-[400px] mb-4 rounded-lg "
          />
        </motion.div>
      </div>
    </section>
