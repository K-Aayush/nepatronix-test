...
const Yt = ({ data }: { data: any }) => {
  ...
  return (
    <Link href={`/tutorials/${data?.link || data?._id}`}>
      <motion.div
        className="relative w-full max-w-[22rem] rounded-xl overflow-hidden bg-indigo-900/30 backdrop-blur-sm border border-white/20 transition-all duration-300"
      ...
      >
        <div className="relative aspect-video">
          ...
          />
          <motion.div
            className="absolute inset-0 flex items-center justify-center transition-opacity duration-300 opacity-0 bg-gradient-to-b from-transparent to-black/50 hover:opacity-100 rounded-t-xl"
            initial={{ opacity: 0 }}
